...
type ParsePrintFormat<S extends string, Res extends any[] = []> = 
  S extends `${infer A}${infer B}${infer R}` ?
    [Extends<A, '%'>, Extends<B, '%'>] extends [true, true] ? ParsePrintFormat<R, Res> :
    [Extends<A, '%'>, Extends<B, keyof ControlsMap>] extends [true, true] ?
      B extends keyof ControlsMap ? ParsePrintFormat<R, [...Res, ControlsMap[B]]> : never
    : ParsePrintFormat<`${B}${R}`, Res>
  : Res
