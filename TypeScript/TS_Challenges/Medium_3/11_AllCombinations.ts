...
type Split<S extends string> = S extends `${infer First}${infer Rest}` ? "" | First | Split<Rest> : ""
type Combinations<C extends string, U  extends string = C> = C extends C ? C | `${C}${Combinations<Exclude<U, C>>}` : never
type AllCombinations<S extends string> = Combinations<Split<S>>

type T = AllCombinations<'ABC'>
